//
//  src/platform/mac/MetalContextMac.mm
//  pTK
//
...
{
    MetalContextMac::MetalContextMac(void* mainView, const Size& size, const Vec2f& scale)
    ...
    MetalContextMac::~MetalContextMac()
    {
        @autoreleasepool
        {
            if (m_context)
            ...
            PTK_INFO("Destroyed MetalContextMac");
        } // autoreleasepool
    }

    void MetalContextMac::init(void* mainView, const Size& size, const Vec2f& scale)
    {
        @autoreleasepool
        {
            m_mainView = static_cast<NSView*>(mainView);
            CGDirectDisplayID viewDisplayID = static_cast<CGDirectDisplayID>(
                [m_mainView.window.screen.deviceDescription[@"NSScreenNumber"] unsignedIntegerValue]);
            ...
            Size fSize{static_cast<Size::value_type>(rect.size.width), static_cast<Size::value_type>(rect.size.height)};

            PTK_INFO("Initialized MetalContextMac");
        } // autoreleasepool
    }

    void MetalContextMac::resize(const Size& size)
    {
        @autoreleasepool
        {
            NSRect rect;
            ...
            setSize(size);
            PTK_INFO("Sized MetalContextMac to {}x{}", size.width, size.height);
        } // autoreleasepool
    }

    sk_sp<SkSurface> MetalContextMac::surface() const
    {
        @autoreleasepool
        {
            ...
            if (m_context)
            {
                ...
                surface = SkSurface::MakeFromBackendRenderTarget(m_context.get(), backendRT, kTopLeft_GrSurfaceOrigin,
                                                                 kBGRA_8888_SkColorType, nullptr, nullptr);

                m_drawableHandle = CFRetain(static_cast<GrMTLHandle>(currentDrawable));
            }
            // surface = SkSurface::MakeFromCAMetalLayer(m_context.get(), m_metalLayer, kTopLeft_GrSurfaceOrigin, 1,
            // kBGRA_8888_SkColorType, nullptr, nullptr, &m_drawableHandle);

            return surface;
        } // autoreleasepool
    }

    void MetalContextMac::swapBuffers()
    {
        @autoreleasepool
        {
            if (m_drawableHandle == nullptr)
